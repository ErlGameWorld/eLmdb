...
basic_example() ->
	...
	%% 2. 打开数据库
	io:format("2. 打开数据库...~n"),
	{ok, DbiRef} = eLmdb:dbOpen(EnvRef, "example_db", 262144), %% MDB_CREATE
	...
	lists:foreach(fun({Key, Value}) ->
		ok = eLmdb:put(EnvRef, DbiRef, Key, Value, 0),
	...
	{ok, DbiStat} = eLmdb:dbStat(EnvRef, DbiRef),
	io:format("   数据库统计: ~p~n", [DbiStat]),
	
	{ok, DbiFlags} = eLmdb:dbFlags(EnvRef, DbiRef),
	io:format("   数据库标志: ~p~n~n", [DbiFlags]),
	
	%% 9. 同步数据到磁盘
	io:format("9. 同步数据到磁盘...~n"),
	ok = eLmdb:envSync(EnvRef),
	io:format("   同步完成~n~n"),
...
batch_operations_example() ->
	...
	%% 创建环境
	{ok, EnvRef} = eLmdb:envOpen(?DB_PATH, ?MAP_SIZE, ?MAX_DBS, ?MAX_READERS, 0),
	{ok, DbiRef} = eLmdb:dbOpen(EnvRef, "batch_db", 262144),
...
traversal_example() ->
	...
	%% 创建环境并插入测试数据
	{ok, EnvRef} = eLmdb:envOpen(?DB_PATH, ?MAP_SIZE, ?MAX_DBS, ?MAX_READERS, 0),
	{ok, DbiRef} = eLmdb:dbOpen(EnvRef, "traversal_db", 262144),
	...
	%% 正序遍历
	io:format("1. 正序遍历...~n"),
	{ok, ForwardData, _} = eLmdb:tvsAsc(EnvRef, DbiRef, '$TvsBegin', 1000),
	lists:foreach(fun({Key, Value}) ->
	...
	%% 反序遍历
	io:format("2. 反序遍历...~n"),
	{ok, ReverseData, _} = eLmdb:tvsDesc(EnvRef, DbiRef, '$TvsBegin', 1000),
	lists:foreach(fun({Key, Value}) ->
		io:format("   ~s -> ~s~n", [Key, Value])
				  end, ReverseData),
	io:format("~n"),
	...
	%% 仅键遍历
	io:format("3. 仅键遍历...~n"),
	{ok, Keys, _} = eLmdb:tvsKeyAsc(EnvRef, DbiRef, '$TvsBegin', 1000),
	lists:foreach(fun(Key) ->
		io:format("   ~s~n", [Key])
				  end, Keys),
	io:format("~n"),
...
performance_example() ->
	...
	{ok, EnvRef} = eLmdb:envOpen(?DB_PATH, ?MAP_SIZE, ?MAX_DBS, ?MAX_READERS, Flags),
	{ok, DbiRef} = eLmdb:dbOpen(EnvRef, "performance_db", 262144),
